...
date_bins <- plot_dat %>% 
  ...
  mutate(site = site %>% str_to_title() %>% fct_recode("Catamount"="Cat"))
...
estimate_bloom_stats <- function(rgnd_df, polygon_area_df){
  rgnd_df %>%
    ...
    mutate(
      cell_count = 22 * rgnd^2, # bil cells / m2
      cell_count_se = 3.5 * rgnd^2,
      cell_area = 694 * rgnd^2, # percent cell area
      cell_area_se = 85 * rgnd^2,
      toc = 16 * rgnd, # g/m2
      toc_se = 2.2 * rgnd,
      tn = 365 * rgnd, # mg/m2
      tn_se = 48 * rgnd,
      delta_albedo = 0.9 * rgnd, # delta albedo
      delta_albedo_se = 0.17 * rgnd, 
...
TOC/TN density
TOC: .88 g/m2 = 880 +/- 121 kg/km2 on Vowell, and 625 +/- 86 on Catamount bloom polygon
TN: 0.02 g/m2 = 20 +/- 2 kg/km2 on Vowell, and 14 +/- 2 on Catamount 

net TOC/TN
TOC: 1480 +/- 204 kg Vowell, 1200 +/- 165 Catamount
TN: 34 +/-4 Vowell, 27 +/-4 Cat
...
Total TOC: 
Total TN: 121+/-16 kg Vowell, 83+/-11 Catamount
...
plot_dat2 %>% 
  ...
  geom_line(stat="smooth", method = "loess", se=FALSE, alpha = 0.7) +
...
my_wx <- ts_custom_wide %>% 
  ...
  mutate(name = name %>% fct_relevel("rgnd", "albedo", "tmax", "tmin", "prcp", "srad") %>% 
           fct_recode("RGND"="rgnd", "MOD10A1 Albedo"="albedo", "Max. Temp C"="tmax", "Min. Temp C"="tmin", "Precip. (mm)" = "prcp", 
...
wxplt <- my_wx %>% 
  ...
  geom_point(alpha = 0.6, data = my_wx %>% filter(name %in% c("RGND", "MOD10A1 Albedo"))) +
  theme_minimal() +
  geom_smooth(color = "red", 
              data = my_wx %>% 
                ...
                bind_rows(tibble(date = ymd("2020-07-01", "2020-07-02", "2020-07-03", "2020-07-04", "2020-07-05"), 
                                 name = "RGND", value=0)) %>% drop_na() %>% 
                mutate(name = name %>% fct_relevel("RGND"))) +
  geom_smooth(data = my_wx %>% filter(name =="MOD10A1 Albedo"), color = "black") +
  geom_line(alpha = 0.5, data = my_wx %>% filter(!(name %in% c("RGND", "MOD10A1 Albedo")))) +
...
```{r}
# ggsave(here("figs/wx_plot.png"),height =9, width = 8 )
```
