...
ppdat %>% 
  ...
  labs(y = "Valid pixel count", x = "Date") +
...
ts_custom_wide %>% 
  ...
  kbl(caption = "Number of cloud-free satellite images per site used in analysis", col.names = c("Platform", "Catamount", "Vowell")) %>% 
...
vowell_aug <- vowell_mod %>% augment(vowell_dat)
...
wxplt <- my_wx %>% 
  ...
  geom_point(alpha = 0.6, shape = 0, color = "#619cff", data = my_wx %>% filter(name %in% c("RGND"))) +
  geom_point(alpha = 0.6, shape = 0, color = "grey30", data = my_wx %>% filter(name %in% c("MOD10A1 Albedo"))) +
  theme_minimal() +
