...
srad <- dir_ls(here("data"), regexp = "*srad_wx.csv$") %>% # doing this seperately for now, redundant since it's also in wx
  map_df(read_csv, id = "filename")
...
pp3 <- plot_dat2 %>% 
  ggplot(aes(x = date, y = rgnd, color = platform, shape = polygon)) +
  facet_grid(rows = vars(site %>% fct_relevel("vowell"))) +
  # MODIS layer
  # geom_point(alpha = 0.5, size = 1, data = plot_dat %>% 
  #             filter(platform == "terra") ) +
...
###compare satellite RGND within 1 day window

```{r}
date_bins <- ts_custom_wide %>% 
  filter(!platform %>% str_detect("Snow")) %>% 
  # bin time into discrete 3 day intervals
  mutate(date_bin = date %>% yday() %>% cut_interval(n=30)) %>% 
...
date_bins %>%
  select(-date_bin, -aqua, -l8) %>% 
  ggpairs(colums = 2:4, ggplot2::aes(colour=site))
```


```{r}
date_bins %>%
  select(-date_bin, -site, -aqua) %>% 
  ggpairs(colums = 2:4)
```
...
```{r}
date_bins %>% 
  select(site, s2, terra) %>% 
  drop_na() %>% 
  ggplot(aes(x = s2, y = terra, color = site)) +
  geom_point() +
  geom_smooth(method = "lm", se=FALSE) +
  labs(x = "Sentinel-2 L2A RGND", y="MODIS/Terra RGND", color = "Site") +
  theme_minimal()
```
...
tt <- max_algae_glims %>% 
  arrange(max_delta_albedo) %>% 
  rename(mean_rgnd = max_rgnd, cell_area_percent_cov = max_frac_red, 
         cell_density_bil_cell_per_m2 = max_cell_density, 
         toc_g_m2 = max_toc, 
         tn_mg_m2 = max_tn, 
         delta_albedo = max_delta_albedo, 
         polygon_area_m2 = glacier_area_m2) %>% 
  pivot_longer(-site) %>% 
  pivot_wider(names_from = site) %>% 
  # control number of sig figs
...
tt2 <- max_algae_custom %>% 
  arrange(max_delta_albedo) %>% 
  rename(mean_rgnd = max_rgnd, 
         cell_area_percent_cov = max_frac_red, 
         cell_density_bil_cell_per_m2 = max_cell_density, 
         toc_g_m2 = max_toc, 
         tn_mg_m2 = max_tn, 
         delta_albedo = max_delta_albedo, 
         polygon_area_m2 = bloom_area_m2) %>% 
  pivot_longer(-site) %>% 
  pivot_wider(names_from = site) %>% 
  select(name, vowell, cat) %>% 
  # control number of sig figs
  mutate_if(is.numeric, format, digits=3,nsmall = 0)
tt2 %>% 
  kbl(format = "latex", booktabs = TRUE) %>% 
