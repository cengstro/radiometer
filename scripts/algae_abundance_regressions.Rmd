...
rgnd_comparison %>% 
  ...
  ggpairs() +
  labs(title = "Field radiometer snow algae sample scan RGND convolution comparison", x = "RGND satellite 1", y="RGND satellite 2")
```
...
```{r}
s2_rgnd %>% 
  ggplot(aes(y = rgnd, x= sample_id %>% fct_reorder(rgnd))) +
  geom_point() +
...
lm models
```{r}
cell_mod <- lm(bil_cells_per_sq_m ~ 0 + rgnd, data = joined_wide_clean) # set intercept to 0 for simplicity
frac_mod <- lm(cell_area_percent ~ 0 + rgnd, data = joined_wide_clean) # set intercept to 0 for simplicity
...
coefs <- bind_rows(
  ...
  relocate(response) %>% 
  # compute the confints (2 standard errors)
  mutate(plus_minus = 3*std.error,
        lower_bound = estimate - 2*std.error,
         upper_bound = estimate + 2*std.error)
coefs
```
95% CI estimates: 
...
frac_poly_mod <- lm(cell_area_percent ~ 0 + poly(rgnd, 2), data = joined_wide_clean)
