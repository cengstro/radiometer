...
# # Ganey 2017 data for direct comparison
# g_cell <- read_csv(here("data/ganey_2017_data/SnowbirdCellData.csv"))
# g_rad <- dir_ls(here("data/ganey_2017_data"), regexp = "*B.csv") %>% 
...
sat_indices %>% 
  select(-planet) %>% my_pairs(ind = "rgnd")
...
s2_l8_compare %>% 
  ...
  ggtitle("S2-L8-Terra RGND comparison (convolved sample scans)")
```
```{r}
s2_l8_compare %>% 
  group_by(name) %>% 
...
resp <- joined_long_clean %>%
  filter(name %in% c("cell_area_percent", "bil_cells_per_sq_m", "cell_cm2_per_l", "toc_g_per_m2", "tn_mg_per_m2")) %>% # properties we might want to predict remotely
...
fig2_dat <- plot_dat %>%
  ...
  mutate(resp_name = resp_name %>% fct_relevel(c("bil_cells_per_sq_m", "cell_area_percent", "toc_g_per_m2", "tn_mg_per_m2"))) # pull(resp_name) %>% levels() #check fct levels

f2 <- fig2_dat %>% 
  ggplot(aes(y = resp_mean, x = mean_rgnd)) +
  geom_smooth(method = "lm", alpha = 0.6, se = FALSE, color = "lightblue", formula = y ~ 0 + x,
              data = fig2_dat %>% filter(resp_name %in% c("cell_area_percent", "bil_cells_per_sq_m"))) + 
...
fig2_dat %>% 
  ...
  facet_grid(rows = vars(resp_name), scales = "free") +
  geom_smooth(method = "lm", alpha = 0.6, color = "lightblue", se = FALSE, formula = y ~ x) +
  geom_point(alpha = 0.4, size = 2) +
  # the x val SD crossbar
  geom_linerange(aes(ymin = resp_mean - resp_sd, 
                      ymax = resp_mean + resp_sd),
...
frac_mod <- lm(cell_area_percent ~ 0 + rgnd, data = mod_dat)
toc_mod <- lm(toc_g_per_m2 ~ rgnd, data = mod_dat)
tn_mod <- lm(tn_mg_per_m2 ~ rgnd, data = mod_dat)
...
frac_poly_mod <- lm(cell_area_percent ~ 0 + poly(rgnd, 2), data = mod_dat)
...
<!--   clean_names() -->
<!-- g_rad %>%  -->
<!--   mutate(value = mean(V1:V10), .keep = "unused") -->
<!-- ``` -->
