...
```{r}
toc_plot_dat <- resp %>% 
  left_join(preds, by = "sample_id") %>% 
  filter(type != "pocket", pred_var == "toc") 

toc_plot_dat %>% 
  ggplot(aes(y = mean_albedo, x = mean)) +
  geom_smooth(method = "lm", se = FALSE, data = toc_plot_dat %>% filter(type=="red"), color = "#F8766D") +
  geom_point(aes(fill = type), color="black", pch = 21, size = 2.4) +
  theme_minimal() +
...
plot_dat_irf <- irf_vis %>% 
  mutate(type = case_when(type=="pocket"~"red",
                          TRUE ~ type)) %>% 
