...
pa <- my_plot(dat, x = mean_cell_area_percent, y = mean_albedo, xsd = sd_cell_area_percent, ysd = sd_albedo, formula = y~log(x)) +
  ...
  # lims(y = c(0, 1)) +
  scale_fill_manual(values = c("gray", "#F8766D", "#766df8", "white"))
pa
```
...
pc <- irf_full %>% 
  drop_na(type) %>% 
  my_plot(x = mean_cell_area_percent, y = irf, xsd = sd_cell_area_percent, ysd = irf_sd, formula = y~log(x)) +
  labs(y = "IRF (Wm2)", x = "Cell area % coverage", fill = "Snow type", tag = "C") +
  scale_fill_manual(values = c("gray", "#F8766D", "#766df8"))
pc
```
